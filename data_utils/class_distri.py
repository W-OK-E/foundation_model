...
case_distributions={}
# iterate
for ci,case in tqdm.tqdm(enumerate(sorted(cases))):
    case_dir=root/case
    ...
    for p in tqdm.tqdm(sorted(case_dir.rglob('*'))):
        if p.is_file() and p.suffix.lower() in {'.png','.jpg','.jpeg','.tif','.tiff'}:
            try:
                im=cv2.cvtColor(cv2.imread(p),cv2.COLOR_BGR2HSV)
...
im = cv2.imread('/mnt/data/omkumar/foundation_phase1/datasets/cataract1k/masks/case_2000/0000000000_rgb_mask.png')

numpy.unique(im.reshape(-1, im.shape[2]), axis=0)