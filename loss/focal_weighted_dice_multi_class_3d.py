...
class WeightedCrossEntropyDiceLoss3D(nn.Module):
    """Weighted CrossEntropy + Dice Loss for multiclass 3D segmentation"""
    def __init__(self, alpha=None, dice_weight=0.5, ignore_index = 0, num_classes = 0):
        super(WeightedCrossEntropyDiceLoss3D, self).__init__()
    ...
    def cross_entropy_loss(self, inputs, targets):
        """CrossEntropy loss with class weights for 3D"""
        # inputs: [B, C, D, H, W], targets: [B, D, H, W]
    ...
    def dice_loss(self, inputs, targets, smooth=1e-5):
        """Dice loss with class weights for multiclass 3D"""
