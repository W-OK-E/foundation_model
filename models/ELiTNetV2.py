...
# Import DCNv4 modules
try:
    from deformable_conv import DeformableConv2dV4, is_dcnv4_available
    DCNV4_AVAILABLE = is_dcnv4_available()
except ImportError:
    DCNV4_AVAILABLE = False
    print("Warning: deformable_conv module not found. DCNv4 mode will not be available.")
...
def get_conv_layer(in_c, out_c, k_sz=3, padding='same', conv_mode='MKConv2D'):
    ...
    Args:
        ...
        conv_mode: Type of convolution ('Conv2d', 'MKConv2D', 'DecomConv2D', 'SeparableConv2d', 'DCNv4')
    
    Returns:
        Convolution layer
    """
    # DCNv4 constraint: requires input channels to be divisible by 16
    # (Output channels handled by 1x1 projection inside DCNv4)
    # Automatically fallback to Conv2d for incompatible channel counts
    if conv_mode == 'DCNv4':
        min_channels_required = 16
        if in_c < min_channels_required or in_c % 16 != 0:
            import warnings
            warnings.warn(
                f"DCNv4 requires input channels >= {min_channels_required} and divisible by 16. "
                f"Got in_channels={in_c}, out_channels={out_c}. "
                f"Using Conv2d for this layer (hybrid approach).",
                UserWarning
            )
            conv_mode = 'Conv2d'  # Fallback to Conv2d for this layer
    ...
    elif conv_mode == 'DCNv4':
        if not DCNV4_AVAILABLE:
            print(f"Warning: DCNv4 not available, falling back to Conv2d")
            return nn.Conv2d(in_c, out_c, kernel_size=k_sz, padding=padding)
        # Use DeformableConv2dV4 directly - normalization/activation handled by ConvBlock
        pad_int = padding if isinstance(padding, int) else (k_sz - 1) // 2
        return DeformableConv2dV4(
            in_c, out_c, 
            kernel_size=k_sz, 
            padding=pad_int,
            bias=False  # No bias since BatchNorm is applied after
        )
    else:
        raise ValueError(f"Unsupported conv_mode: {conv_mode}. Choose from ['Conv2d', 'MKConv2D', 'DecomConv2D', 'SeparableConv2d', 'DCNv4']")

class ConvBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, shortcut=False, pool=True, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        '''
        pool_mode can be False (no pooling) or True ('maxpool')
        conv_mode: Type of convolution ('Conv2d', 'MKConv2D', 'DecomConv2D', 'SeparableConv2d', 'DCNv4')
        apply_dropout: Whether to apply dropout after each convolution
...
class ResBlock(nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        ...
        if self.apply_dropout:
            ...
            self.dropout = nn.Dropout(0.2)  # Keep backward compatibility with hardcoded dropout
...
class Trunk(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, residual=False, causal=False, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        '''
        pool_mode can be False (no pooling) or True ('maxpool')
        conv_mode: Type of convolution ('Conv2d', 'MKConv2D', 'DecomConv2D', 'SeparableConv2d', 'DCNv4')
        apply_dropout: Whether to apply dropout after each convolution
        ...
        if residual:   
            self.conv = nn.Sequential(
                ResBlock(in_c, out_c, k_sz=k_sz, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate),
                ResBlock(out_c, out_c, k_sz=k_sz, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate),
            )
        else:
            if causal:
                layers = [
...
class AttConvBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, shortcut=False, pool='pool', attention=False, residual=False, causal=False, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        '''
        pool_mode can be False (no pooling) or True ('maxpool')
        conv_mode: Type of convolution ('Conv2d', 'MKConv2D', 'DecomConv2D', 'SeparableConv2d', 'DCNv4')
        apply_dropout: Whether to apply dropout after each convolution
        ...
        self.conv = Trunk(in_c, out_c, k_sz=k_sz, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
...
class DoubleAttBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, shortcut=True, pool='pool', attention=True, residual=False, causal=False, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        super(DoubleAttBlock, self).__init__()
        
        self.attention = attention
        self.block1 = AttConvBlock(in_c, in_c, k_sz=k_sz,
                              shortcut=shortcut, pool=False, attention=False, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
        self.block2 = AttConvBlock(in_c, out_c, k_sz=k_sz,
                              shortcut=shortcut, pool=pool, attention=self.attention, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
...
class ConvBridgeBlock(torch.nn.Module):
    def __init__(self, channels, k_sz=3, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        super(ConvBridgeBlock, self).__init__()
        self.block = ResBlock(channels, channels, k_sz=k_sz, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
...
class UpConvBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, up_mode='up_conv', conv_bridge=False, shortcut=False, skip_conn=True, residual=False, causal=False, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        super(UpConvBlock, self).__init__()
        self.conv_bridge = conv_bridge
        self.skip_conn=skip_conn
        self.up_layer = UpsampleBlock(in_c, out_c, up_mode=up_mode)
        self.conv_layer1 = AttConvBlock(out_c, out_c, k_sz=k_sz, shortcut=shortcut, pool=False, attention=True, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
        self.conv_layer2 = AttConvBlock(2 * out_c, out_c, k_sz=k_sz, shortcut=shortcut, pool=False, attention=True, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
        if self.conv_bridge:
            self.conv_bridge_layer = ConvBridgeBlock(out_c, k_sz=k_sz, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
...
class ELiTNetEncoder(nn.Module):
    def __init__(
        ...
        self.first = ConvBlock(in_c=in_c, out_c=layers[0], k_sz=k_sz,
                               shortcut=shortcut, pool=False, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
        ...
        for i in range(len(layers) - 1):
            if i <= 7:
                block = DoubleAttBlock(in_c=layers[i], out_c=layers[i + 1], k_sz=k_sz,
                                shortcut=shortcut, pool=pool, attention=True, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
            else:
                block = DoubleAttBlock(in_c=layers[i], out_c=layers[i + 1], k_sz=k_sz,
                                shortcut=shortcut, pool=pool, attention=False, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
    ...
    def __init__(
        ...
        for i in range(len(layers) - 1):
            block = UpConvBlock(in_c=reversed_layers[i], out_c=reversed_layers[i + 1], k_sz=k_sz,
                                ...
                                residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
...
class EliTNet2D(nn.Module):
    def __init__(self, in_c, n_classes, layers, k_sz=3, up_mode='pixelshuffle', pool='pool', 
                 conv_bridge=True, shortcut=True, skip_conn=True, residual=False, causal=False, conv_mode='MKConv2D', apply_dropout=False, dropout_rate=0.2):
        """
        ...
        Args:
            ...
            conv_mode: Type of convolution to use ('Conv2d', 'MKConv2D', 'DecomConv2D', 'SeparableConv2d', 'DCNv4')
            apply_dropout: Whether to apply dropout after each convolution operation
            dropout_rate: Dropout probability (default: 0.2)
        """
        super(EliTNet2D, self).__init__()
        #self.n_classes = n_classes
        self.encoder = ELiTNetEncoder(in_c, k_sz, layers, pool=pool, residual=residual, causal=causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
        #self.latent = nn.Conv1d(widths[-1],latent_features,1)
        self.decoder = ELiTNetDecoder(n_classes, k_sz, layers, up_mode, conv_bridge, shortcut, skip_conn, residual, causal, conv_mode=conv_mode, apply_dropout=apply_dropout, dropout_rate=dropout_rate)
