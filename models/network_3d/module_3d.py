...
from separableconv.nn import SeparableConv3d
from .utils import generate_series, select_n_elements_with_repeats_and_fallback

class DecomConv3D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, conv_type='conv', stride=1, padding='same', dilation = 1, bias=True):
        super(DecomConv3D, self).__init__()
        conv_layers = {
            'conv': nn.Conv3d,
            'sep': SeparableConv3d,  # Replace with actual SeparableConv2d implementation
            'dwconv': nn.Conv3d,  # Depthwise convolution will be handled by setting groups=in_channels
        }
...
class MKConv3D(nn.Module):
    ...
    def _get_conv_layer(self, i):       
        if self.kernel_sizes[i] > 10:
            return DecomConv3D(self.in_channels, 1, kernel_size=self.kernel_sizes[i], conv_type = self.decom_conv, stride=self.stride, padding=self.padding, bias=self.bias)
        else:
            return nn.Conv3d(self.in_channels, 1, kernel_size=self.kernel_sizes[i], stride=self.stride, padding=self.padding, bias=self.bias)
