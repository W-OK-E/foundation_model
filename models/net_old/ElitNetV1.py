...
class ElitNet(nn.Module):
    ...
    def forward(self, x):
        print("Inside the network:",torch.any(torch.isnan(x)))
        x = self.first(x)
        print("After the first forward pass:",torch.any(torch.isnan(x)))
        down_activations = []
        for i, down in enumerate(self.down_path):
            down_activations.append(x)
            x = down(x)
            print("Down Activations:",torch.any(torch.isnan(x)))
        down_activations.reverse()
        for i, up in enumerate(self.up_path):
            x = up(x, down_activations[i])
            print("Up conv:",torch.any(torch.isnan(x)))
        final_x = self.final(x)
        print("Final Output:",torch.any(torch.isnan(final_x)))
        return self.final(x)      